...
class Credit_note_model extends MY_Model
{
    ...
    public function get_credit_notes_with_date($filterBy = null, $from_date = null, $to_date = null)
    {
        $to_date = $to_date ?? date('Y-m-d');
        if (!empty($from_date)) {
            $all_invoice = get_result('tbl_credit_note', array('credit_note_date >=' => $from_date, 'credit_note_date <=' => $to_date));
        } else {
            $all_invoice = $this->get_permission('tbl_credit_note');
        }
        if (empty($filterBy) || !empty($filterBy) && $filterBy == 'all') {
