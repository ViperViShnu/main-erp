...
class Invoice_Model extends MY_Model
{
    ...
    public function get_invoices_with_date($filterBy = null, $from_date = null, $to_date = null)
    {
        ...
        if (!empty($from_date)) {
            $all_invoice = get_result('tbl_invoices', array('invoice_date >=' => $from_date, 'invoice_date <=' => $to_date));
        } else {
            $all_invoice = $this->get_permission('tbl_invoices');
        }
    ...
    public function get_invoice_report($filterBy = null, $range = null, $notes)
    {
        if (!empty($filterBy) && is_numeric($filterBy)) {
            $invoice = $this->db->where('client_id', $filterBy)->get('tbl_invoices')->result();
        } elseif(!empty($filterBy) && $filterBy == 'Directly from purchase' && $notes == 'Directly from purchase') {
            $invoice = $this->db->where('notes', $filterBy)->get('tbl_invoices')->result();
        } elseif(!empty($notes) && $filterBy != 'Directly from purchase' && $notes == 'Directly from purchase') {
            $all_data = $this->db->where(array('notes' => $notes))->get('tbl_invoices')->result();
        } else {
