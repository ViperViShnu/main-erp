...
class Attendance extends Admin_Controller
{
    ...
    public function get_mytime_info($attendance_info)
    {
        ...
        if (!empty($attendance_info)) {
            foreach ($attendance_info as $v_info) {
                $date_in = strtotime($v_info->date_in);
                $date_out = strtotime($v_info->date_out);

                if ($date_in == $date_out) {
                    // If date_in and date_out are the same, handle it as a single day
                    $date = strftime(config_item('date_format'), $date_in);
                    $clock_info[date('Y', $date_in)][date('W', $date_in)][$date] = get_result('tbl_attendance', array('attendance_id' => $v_info->attendance_id));
                } else {
                    // If date_in and date_out are different, handle each day separately
                    for ($current_date = $date_in; $current_date <= $date_out; $current_date = strtotime('+1 day', $current_date)) {
                        $date = strftime(config_item('date_format'), $current_date);
                        $day_start = ($current_date == $date_in) ? date('H:i:s', $date_in) : '00:00:00';
                        $day_end = ($current_date == $date_out) ? date('H:i:s', $date_out) : '23:59:59';

                        $attendance_data = array(
                            'date' => $date,
                            'start_time' => $day_start,
                            'end_time' => $day_end,
                            'attendance_id' => $v_info->attendance_id
                        );

                        $clock_info[date('Y', $current_date)][date('W', $current_date)][$date] = $attendance_data;
                    }
                }
            }

            // Calculate total time
            array_walk_recursive($clock_info, function (&$attendance_data, $date) {
                if (is_array($attendance_data)) {
                    $total_seconds = array_reduce($attendance_data, function ($carry, $entry) {
                        if (is_array($entry)) {
                            $start_time = strtotime($entry['date'] . ' ' . $entry['start_time']);
                            $end_time = strtotime($entry['date'] . ' ' . $entry['end_time']);
                            return $carry + ($end_time - $start_time);
                        }
                        return $carry;
                    }, 0);
                    ...
                    $attendance_data['total_time'] = "{$total_hours} hours {$total_minutes} minutes";
                }
            });
        }
    ...
    public function get_mytime_info_old($attendance_info)
    {
        if (!empty($attendance_info)) {
            foreach ($attendance_info as $v_info) {
                if ($v_info->date_in == $v_info->date_out) {
                    ...
                    $date = 'From' . ' : ' . strftime(config_item('date_format'), strtotime($v_info->date_in)) . ', ' . 'To' . ' : ' . strftime(config_item('date_format'), strtotime($v_info->date_out));
                }
    ...
    public function saved_manual_time_new() // 13-08-2024 Changed by Vishnu
    {
        $adata = $this->attendance_model->array_from_post(array('user_id', 'date_in', 'date_out'));
        $date_in = date('Y', strtotime($adata['date_in']));
        if (empty($adata['date_in'])) {
        ...
        // Get all dates between date_in and date_out
        $dates_between = $this->get_dates_between($adata['date_in'], $adata['date_out']);

        foreach($dates_between as $date) {
            $check_date = $this->attendance_model->check_by(array('user_id' => $adata['user_id'], 'date_in' => $date), 'tbl_attendance');
    ...
    private function get_dates_between($start_date, $end_date)
    {
        $dates = [];
    ...
    public function timechange_requestList($type = null)
    {
        if ($this->input->is_ajax_request()) {
            ...
            foreach ($fetch_data as $_key => $v_clock_history) {
                ...
                $date_in = null;
                if(!empty($v_clock_history->date_in)) {
                    $date_in = $v_clock_history->date_in;
                } else {
                    $date_in = '-';
                }
                $sub_array[] = $date_in;
                ...
                $date_out = null;
                if(!empty($v_clock_history->date_out)) {
                    $date_out = $v_clock_history->date_out;
                } else {
                    $date_out = '-';
                }
                $sub_array[] = $date_out;
