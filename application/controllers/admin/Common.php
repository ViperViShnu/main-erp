...
class Common extends MY_Controller
{
    ...
    public function items_suggestions($limit = null)
    {
        ...
        if (!empty($warehouse_id)) {
            $rows = $this->admin_model->getItemsInfoNew($term, $warehouse_id, $limit);
        }
        ...
        if (!empty($rows)) {
            ...
            foreach ($rows as $row) {
                if (!isset($groupedItems[$row->customer_group_id])) {
                    $groupedItems[$row->customer_group_id] = [];
                }
                $groupedItems[$row->customer_group_id][] = $row;

                if (!isset($customerGroups[$row->customer_group_id])) {
                    $customerGroups[$row->customer_group_id] = $row->customer_group;
                }
            }

            $tabsHtml = '
                <ul class="nav nav-tabs" style="border-bottom: 1px solid #ddd; padding-bottom: 10px; margin-bottom: 10px;">
                    <li class="nav-item active" style="margin-right: 5px;">
                        <a class="nav-link" data-toggle="tab" href="#group_all" padding: 10px 15px; border-radius: 4px; font-size: 14px;">All</a>
                    </li>';

            foreach ($customerGroups as $groupId => $groupName) {
                $tabsHtml .= '
                    <li class="nav-item" style="margin-right: 5px;">
                        <a class="nav-link" data-toggle="tab" href="#group_' . $groupId . '" style="padding: 10px 15px; border-radius: 4px; font-size: 14px;">' . htmlspecialchars($groupName) . '</a>
                    </li>';
            }
            $tabsHtml .= '</ul>';

            $html .= '<div class="tab-content" style="padding: 0px";>';

            $html .= '<div id="group_all" class="tab-pane fade active in">';
            foreach ($rows as $row) {
                $imageURL = base_url('assets/img/product.png');
                if (!empty($row->upload_file)) {
                    ...
                    if ($uploaded_file[$rand_keys]->is_image == 1) {
                        $imageURL = base_url($uploaded_file[$rand_keys]->path);
                    } else {
                        $imageURL = base_url('assets/img/filepreview.jpg');
                    }
                }
                $html .= '<div class="col-xs-2 p0 m0 item" style="max-height:108px;overflow: hidden"> 
                            <a class="' . $class . ' btn p0" data-item-id="' . $row->saved_items_id . '">
                                <img class="round" src="' . $imageURL . '" style="height: 70px;width: 100%"> 
                                <div class="text-xs-center text"> 
                                    <small style="white-space: pre-wrap;">' . htmlspecialchars($row->item_name) . ' ' . htmlspecialchars($row->code) . '</small>
                                </div>
                            </a>
                          </div>';
            }
            ...
            foreach ($groupedItems as $groupId => $items) {
                $html .= '<div id="group_' . $groupId . '" class="tab-pane fade">';
        ...
        echo json_encode($tabsHtml . $html);
        exit();
    }
    ...
    public function getItemByWarehouse($warehouseId = NULL)
    {
        ...
        if (!empty($warehouseId)) {
            $rows = $this->admin_model->getItemsInfoNew('', $warehouseId);
        }
        ...
        if (!empty($rows)) {
            ...
            foreach ($rows as $row) {
                ...
                $html .= '<div class="col-xs-2 p0 m0" style="max-height:108px;overflow: hidden"> <a class="' . $class . ' btn p0" data-item-id="' . $row->saved_items_id . '">
                                            <img class="round" src="' . $imgaeURL . '" style="height: 70px;width: 100%"> <div class="text-xs-center text"> <small style="white-space: pre-wrap;">' . $row->item_name . ' ' . $row->code . '</small></div></a></div>';
            }
            $html .= '</div>';
