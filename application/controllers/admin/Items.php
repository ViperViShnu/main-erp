...
class Items extends Admin_Controller
{
    ...
    public function itemsList($group_id = null, $type = null)
    {
        if ($this->input->is_ajax_request()) {
            ...
            foreach ($fetch_data as $_key => $v_items) {
                ...
                $inventory = null;
                $warehouse_products_info = $this->db->where('product_id', $v_items->saved_items_id)->get('tbl_warehouses_products')->result();

                if(!empty($warehouse_products_info)) {
                    foreach($warehouse_products_info as $key => $products_info) {
                        if(!empty($products_info->warehouse_id)) {
                            $warehouse_name = $this->db->where('warehouse_id', $products_info->warehouse_id)->get('tbl_warehouse')->row()->warehouse_name;

                            if(count($warehouse_products_info) > 1) {
                                $inventory .= $key + 1 . '.&nbsp' . $warehouse_name . '&nbsp;:&nbsp;' . number_format($products_info->quantity, 2).'<br>';
                            } else {
                                // $inventory .= 'Warehouse : ' . $warehouse_name . ' <br> Quantity : ' . $products_info->quantity;
                                $inventory .= $warehouse_name . '&nbsp;:&nbsp;' . number_format($products_info->quantity, 2);
                            }
                        }
                    }
                }
    ...
    public function save_importedOld()
    {
        ...
        if (!empty($files)) {
            ...
            foreach ($files as $key => $value) {
                ...
                $data['code'] = (!empty($value[1]) ? $value[1] : '');
                $data['item_desc'] = (!empty($value[2]) ? $value[2] : '');
                $data['quantity'] = trim($value[3]);
                $cost_price = str_replace(',', '.', trim($value[4]));
                $data['cost_price'] = preg_replace("/[^0-9,.]/", "", $cost_price);
                $unit_cost = str_replace(',', '.', trim($value[5]));
                $data['unit_cost'] = preg_replace("/[^0-9,.]/", "", $unit_cost);
                $data['unit_type'] = (!empty($value[6]) ? $value[6] : '');
                $data['barcode_symbology'] = 'code128';
                $taxtname = $this->remove_numbers($value[7]);
                ...
                $warehouse_name = trim($value[9]);
                $warehouse_info = $this->items_model->check_by(array('warehouse_name' => $warehouse_name), 'tbl_warehouse');
                $warehouse_ids[] = $warehouse_info->warehouse_id;
                $all_data[] = $data;
            }

            if (!empty($all_data)) {
                $this->db->insert_batch('tbl_saved_items', $all_data);
                $item_names = array_column($all_data, 'item_name');
                $inserted_ids = $this->db->select('saved_items_id')
                    ->where_in('item_name', $item_names)
                    ->get('tbl_saved_items')
                    ->result_array();
                $saved_ids = array_column($inserted_ids, 'saved_items_id');

                foreach($saved_ids as $key => $id) {
                    $check = $this->items_model->check_by(array('product_id' => $id), 'tbl_warehouses_products');
                    $_data['warehouse_id'] = $warehouse_ids[$key];
                    $_data['product_id'] = $id;
                    $this->items_model->_table_name = 'tbl_warehouses_products';
                    $this->items_model->_primary_key = 'id';
                    if(!empty($check)) {
                        $_data['quantity'] = $all_data[$key]['quantity'] + $check->quantity;
                        $this->items_model->save($_data, $check->id);
                    } else {
                        $_data['quantity'] = $all_data[$key]['quantity'];
                        $this->items_model->save($_data);
                    }
                }
            }
    ...
    public function save_imported()
    {
        ...
        // Check if there is an error during file import
        if (!empty($files['message'])) {
            set_message('error', $files['message']);
            redirect('admin/items/import');
        }
        ...
        if (!empty($files)) {
            $all_data = array();
            $warehouse_ids = array();
            $error = []; // Initialize error array for messages
            ...
            foreach ($files as $key => $value) {
                ...
                if ($warehouse_info) {
                    ...
                    $error[] = "Warehouse not found for: " . $warehouse_name; // Store error if warehouse not found
                }
                
                $all_data[] = $data;
            }

            // After processing all rows, check for existing items and insert data
            if (!empty($all_data)) {
                foreach ($all_data as $key => $data) {
                    $where = array('item_name' => $data['item_name']);
                    $check_items = $this->items_model->check_update('tbl_saved_items', $where);
                    if (!empty($check_items)) {
                        $msg = "<strong style='color:#000'>" . $data['item_name'] . "</strong> " . lang('already_exist');
                        $error[] = $msg; // Add error for existing items
                        unset($all_data[$key]); // Remove duplicates
                        $new_data = array_values($all_data);
                    } else {
                        $new_data = array_values($all_data);
                    }
                }
                ...
                if (!empty($new_data)) {
                    // Insert data into 'tbl_saved_items'
                    $this->db->insert_batch('tbl_saved_items', $new_data);
                    $item_names = array_column($new_data, 'item_name');

                    // Retrieve the inserted item IDs
                    $inserted_ids = $this->db->select('saved_items_id')
                        ->where_in('item_name', $item_names)
                        ->get('tbl_saved_items')
                        ->result_array();
                    ...
                    foreach ($saved_ids as $key => $id) {
                        ...
                        $check = $this->items_model->check_by(['product_id' => $id], 'tbl_warehouses_products');
                        $this->items_model->_table_name = 'tbl_warehouses_products';
                        $this->items_model->_primary_key = 'id';
                        if (!empty($check)) {
                            $_data['quantity'] = $new_data[$key]['quantity'] + $check->quantity;
                            $this->items_model->save($_data, $check->id);
                        } else {
                            $_data['quantity'] = $new_data[$key]['quantity'];
                            $this->items_model->save($_data);
                        }
                    }

                    // Add success message for inserted items
                    $success = array_column($new_data, 'item_name');
                    if (!empty($success)) {
                        $type = 'success';
                        $message = lang('save_new_items');
                    }
            ...
            if (!empty($error)) {
                foreach ($error as $err_msg) {
                    set_message('error', $err_msg); // Show error message
                }
            }

            // Redirect to items list after processing
            set_message($type, $message);
            redirect('admin/items/items_list');
        } else {
            $type = 'error';
            $message = "Sorry your uploaded file type not allowed! Please upload an XLS/CSV file.";
            set_message($type, $message);
            redirect('admin/items/items_list');
        }
    }
