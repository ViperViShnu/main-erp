    ...
    public function warehouseList()
    {
        if ($this->input->is_ajax_request()) {
            ...
            foreach ($fetch_data as $key => $row) {
                ...
                $product = null;
                $products_info = $this->db->where('warehouse_id', $row->warehouse_id)->get('tbl_warehouses_products')->result();

                if(!empty($products_info)) {
                    foreach($products_info as $key => $info) {
                        if(!empty($info->product_id)) {
                            $product_name = $this->db->where('saved_items_id', $info->product_id)->get('tbl_saved_items')->row()->item_name;

                            if(count($products_info) > 1) {
                                $product .= $key +1 . '.&nbsp;' . $product_name . '&nbsp;:&nbsp;' . number_format($info->quantity, 2) . '<br>';
                            } else {
                                $product .= $product_name . '&nbsp;:&nbsp;' . number_format($info->quantity, 2);
                            }
                        }
                    }
                }
