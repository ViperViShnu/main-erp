    ...
    <div class="media-list-container <?= (!empty($attach_list) && $attach_list == 'list_view' ? 'hidden' : '') ?>">
        ...
        if (!empty($files_info)) {
            foreach ($files_info as $key => $v_files_info) {
                ...
                ?>
                <div class="panel-group" id="media_list_container-<?= $files_info[$key][0]->attachments_id ?>"
                            ...
                            <h4 class="panel-title">
                                <a data-toggle="collapse" data-parent="#accordion" href="#<?php echo $key ?>"
                                   aria-expanded="true" aria-controls="collapseOne">
                                    <strong class="text-alpha-inverse"><?php echo $files_info[0]->title; ?> </strong>
                                    <small style="color:#ffffff " class="pull-right">
                                        <?php if ($files_info[0]->user_id == $this->session->userdata('user_id')) { ?>
                                            <?php echo ajax_anchor(base_url("client/projects/delete_files/" . $files_info[0]->attachments_id), "<i class='text-danger fa fa-trash-o'></i>", array("class" => "", "title" => lang('delete'), "data-fade-out-on-success" => "#media_list_container-" . $files_info[0]->attachments_id)); ?>
                                        <?php } ?></small>
                                </a>
                            </h4>
                        </div>
                        <div id="<?php echo $key ?>" class="panel-collapse collapse <?php
                        if (!empty($in) && $files_info[0]->files_id == $in) {
                            echo 'in';
                        }
