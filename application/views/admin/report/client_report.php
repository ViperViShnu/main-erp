            ...
            if (!empty($profile_info)) {
                if ($profile_info->company == $v_client->client_id) {
                    if ($v_tickets->status == 'answered') {
                        $tickets_answered += count((array)$v_tickets->status);
                    }
                    if ($v_tickets->status == 'closed') {
                        $tickets_closed += count((array)$v_tickets->status);
                    }
                    if ($v_tickets->status == 'open') {
                        $tickets_open += count((array)$v_tickets->status);
                    }
                    if ($v_tickets->status == 'in_progress') {
                        $tickets_in_progress += count((array)$v_tickets->status);
                    }
                }
            }
        }
